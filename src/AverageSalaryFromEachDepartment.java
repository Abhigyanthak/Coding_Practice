...
public class AverageSalaryFromEachDepartment {
    public static void main(String args []) {
        ...
        Map<String, Double> filtered = empList.stream().filter(e-> "M".equalsIgnoreCase(e.getGender())).collect(Collectors.groupingBy(
                Employees::getDeptName,
                Collectors.averagingLong(Employees::getSalary)
        ));
        System.out.println(filtered);

        long salary = empList.stream().filter(e->"M".equalsIgnoreCase(e.getGender())).sorted(Comparator.comparing(Employees::getSalary).reversed())
                .skip(1)
                .findFirst()
                .get()
                .getSalary();
